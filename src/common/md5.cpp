...
namespace {
  ...
  const std::array<std::uint32_t, 64> S = {
   ...
   * 512
   */
  const std::array<std::uint8_t, 64> PADDING = {
 ...
 */
void MD5::Update(
  MD5Context* ctx, const std::span<const std::uint8_t>& input_buffer,
      ...
      for(std::uint32_t j = 0; j < 16; ++j) {
        ...
        input[j] = static_cast<std::uint32_t>(ctx->input[(j * 4) + 3]) << 24U
                   ...
                   | static_cast<std::uint32_t>(
                     ctx->input[(static_cast<size_t>(j * 4))]
                   );
      }
...
void MD5::Finalize(MD5Context* ctx) {
  ...
  for(std::uint32_t j = 0; j < 14; ++j) {
    input[j] =
      static_cast<std::uint32_t>(ctx->input[(j * 4) + 3]) << 24U
      | static_cast<std::uint32_t>(ctx->input[(j * 4) + 2]) << 16U
      | static_cast<std::uint32_t>(ctx->input[(j * 4) + 1]) << 8U
  ...
  for(std::uint32_t i = 0; i < 4; ++i) {
    ctx->digest[(i * 4) + 0] =
      static_cast<std::uint8_t>((ctx->buffer[i] & 0x000000FFU));
    ctx->digest[(i * 4) + 1] =
      static_cast<std::uint8_t>((ctx->buffer[i] & 0x0000FF00U) >> 8U);
    ctx->digest[(i * 4) + 2] =
      static_cast<std::uint8_t>((ctx->buffer[i] & 0x00FF0000U) >> 16U);
    ctx->digest[(i * 4) + 3] =
      static_cast<std::uint8_t>((ctx->buffer[i] & 0xFF000000U) >> 24U);
  }
}
 ...
 */
void MD5::Step(
  std::array<std::uint32_t, 4>& buffer, std::array<std::uint32_t, 16>& input
) {
