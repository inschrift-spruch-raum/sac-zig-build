...
public:
  ...
  double r_01open() { // (0,1)
    return std::uniform_real_distribution<double>{
      std::nextafter(0.0, std::numeric_limits<double>::max()), 1.0
    }(engine);
  };

  double r_01closed() { // [0,1]
    return std::uniform_real_distribution<double>{
      0, std::nextafter(1.0, std::numeric_limits<double>::max())
    }(engine);
  };

  double r_int(double imin, double imax) { // double in [imin,imax]
    return std::uniform_real_distribution<double>{
      imin, std::nextafter(imax, std::numeric_limits<double>::max())
    }(engine);
  };

  std::uint32_t ru_int(
    std::uint32_t imin, std::uint32_t imax
  ) { // std::int32_t in [imin,imax]
  ...
  bool event(double p) { return r_01() < p; };
