...
{
  return ((static_cast<std::int32_t>(static_cast<std::uint32_t>(numbytes)&1U)) != 0)?(numbytes+1):numbytes;
}
} // namespace

void Chunks::Append(std::uint32_t chunkid,std::uint32_t chunksize,const std::span<const std::uint8_t> data)
{
  tChunk chunk;
  chunk.id=chunkid;
...
std::size_t Chunks::PackMetaData(std::vector <std::uint8_t>&data)
{
  ...
  for (std::size_t i=0;i<GetNumChunks();i++) {
    ...
    BitUtils::put32LH(std::span<std::uint8_t, 4>(&data[ofs], 4),wavchunk.id);ofs+=4;
    BitUtils::put32LH(std::span<std::uint8_t, 4>(&data[ofs], 4), wavchunk.csize);ofs+=4;
    std::copy(begin(wavchunk.data),end(wavchunk.data),begin(data)+static_cast<std::int32_t>(ofs));
...
std::size_t Chunks::UnpackMetaData(const std::vector <std::uint8_t>&data)
{
  std::size_t ofs=0;
  while (ofs<data.size()) {
    std::uint32_t chunkid=BitUtils::get32LH(std::span<const std::uint8_t, 4>(&data[ofs], 4));ofs+=4;
    std::uint32_t chunksize=BitUtils::get32LH(std::span<const std::uint8_t, 4>(&data[ofs], 4));ofs+=4;
    if (chunkid==0x46464952) {Append(chunkid,chunksize,std::span<const std::uint8_t>{&data[ofs], 4});ofs+=4;}
    else if (chunkid==0x61746164) {Append(chunkid,chunksize,std::span<const std::uint8_t, 0>{});}
    else {
        const std::uint32_t writesize=word_align(static_cast<std::int32_t>(chunksize));
        Append(chunkid,chunksize,std::span<const std::uint8_t>{&data[ofs], writesize});ofs+=writesize;
    };
  }
  return ofs;
}
...
std::expected<void, AudioFileErr::Err> Wav<AudioFileBase::Mode::Read>::ReadHeader() {
  ...
  if (chunkid==0x46464952 && BitUtils::get32LH(std::span<std::uint8_t, 4>(&buf[8],4))==0x45564157) {

    myChunks.Append(chunkid,chunksize,std::span<const std::uint8_t>{&buf[8], 4});
    while (true) {
      ...
      if (chunkid==0x020746d66) { // read 'fmt ' chunk
        if (chunksize!=16 && chunksize!=18 && chunksize!=40)
        {
          ...
          myChunks.Append(chunkid,chunksize,std::span<const std::uint8_t>{buf.data(),chunksize});

          std::int32_t audioformat=BitUtils::get16LH(std::span<std::uint8_t, 2>(buf.data(), 2));
          numchannels=BitUtils::get16LH(std::span<std::uint8_t, 2>(&buf[2], 2));
          samplerate=static_cast<std::int32_t>(BitUtils::get32LH(std::span<std::uint8_t, 4>(&buf[4], 4)));
          byterate=static_cast<std::int32_t>(BitUtils::get32LH(std::span<std::uint8_t, 4>(&buf[8], 4)));
          blockalign=BitUtils::get16LH(std::span<std::uint8_t, 2>(&buf[12], 2));
          bitspersample=BitUtils::get16LH(std::span<std::uint8_t, 2>(&buf[14], 2));
          if (chunksize>=18) {
             std::int32_t cbsize=BitUtils::get16LH(std::span<std::uint8_t, 2>(&buf[16], 2));
             if (verbose) { std::cout << "  WAVE-Ext (" << cbsize << " Bytes)"<<'\n';}
             if (cbsize>=22) {
               std::int32_t valid_bitspersample=BitUtils::get16LH(std::span<std::uint8_t, 2>(&buf[18], 2));
               auto channel_mask=static_cast<std::int32_t>(BitUtils::get32LH(std::span<std::uint8_t, 4>(&buf[20], 4)));
               std::int32_t data_fmt=BitUtils::get16LH(std::span<std::uint8_t, 2>(&buf[24], 2));
               if (verbose) {
        ...
        endofdata=datapos+std::streampos(word_align(static_cast<std::int32_t>(chunksize)));
        //std::cout << endofdata << ' ' << filesize << '\n';
        if (endofdata>=filesize) { // if data chunk is last chunk, break
            if (endofdata>filesize) {
              numsamples = static_cast<std::int32_t>( (filesize-datapos) / static_cast<std::int64_t>(blockalign));
              samplesleft = numsamples;
        ...
        const std::uint32_t readsize=word_align(static_cast<std::int32_t>(chunksize));
        Read(vbuf,readsize);
        myChunks.Append(chunkid,chunksize,std::span<const std::uint8_t>{vbuf.data(),readsize});
      }
  ...
  if (verbose) {
    ...
    for (std::size_t i=0;i<myChunks.GetNumChunks();i++) {
      std::cout << "  Chunk" << std::setw(2) << (i+1) << ": '" << BitUtils::U322Str(myChunks.GetChunkID(static_cast<std::int32_t>(i))) << "' " << myChunks.GetChunkSize(static_cast<std::int32_t>(i)) << " Bytes\n";}
...
std::int32_t Wav<AudioFileBase::Mode::Read>::ReadSamples(std::vector <std::vector <std::int32_t>>&data,std::int32_t samplestoread) {
  // read samples
  ...
  if (csize==1) {
    ...
    for (std::int32_t i=0;i<samplesread;i++) { // unpack samples
      for (std::int32_t k=0;k<numchannels;k++) {
        std::uint8_t sample=filebuffer[bufptr];bufptr+=1;
    ...
    for (std::int32_t i=0;i<samplesread;i++) { // unpack samples
      for (std::int32_t k=0;k<numchannels;k++) {
        auto sample = static_cast<std::int16_t>(static_cast<std::uint16_t>(filebuffer[bufptr + 1] << 8U) | static_cast<std::uint16_t>(filebuffer[bufptr]));
        bufptr += 2;
...
:WavBase(verbose), AudioFile(fname, file) {
  kbps=(samplerate*numchannels*bitspersample)/1000;
  auto csize=static_cast<std::int32_t>(std::ceil(static_cast<double>(bitspersample)/8.));
...
std::int32_t Wav<AudioFileBase::Mode::Write>::WriteSamples(const std::vector <std::vector <std::int32_t>>&data,std::int32_t samplestowrite) {
  const std::int32_t csize=blockalign/numchannels;
  if (csize==1) {
    std::int32_t bufptr=0;
      for (std::int32_t i=0;i<samplestowrite;i++) { // pack samples
        for (std::int32_t k=0;k<numchannels;k++) {
          filebuffer[bufptr]=static_cast<std::uint8_t>( static_cast<std::uint32_t>(data[k][i]+128) &0xffU);
          bufptr+=1;
      }
    }
  } else if (csize==2) {
    std::int32_t bufptr=0;
      for (std::int32_t i=0;i<samplestowrite;i++) { // pack samples
        for (std::int32_t k=0;k<numchannels;k++) {
          ...
          filebuffer[bufptr+1]=static_cast<std::uint16_t>(static_cast<std::uint16_t>(sample) >> 8U)&0xffU;
          bufptr+=2;
      }
    }
  } else if (csize==3) {
    std::int32_t bufptr=0;
      for (std::int32_t i=0;i<samplestowrite;i++) { // pack samples
        for (std::int32_t k=0;k<numchannels;k++) {
          ...
          filebuffer[bufptr+1]=(static_cast<std::uint32_t>(sample)>>8U)&0xffU;
          filebuffer[bufptr+2]=(static_cast<std::uint32_t>(sample)>>16U)&0xffU;
          bufptr+=3;
      }
    }
  }
