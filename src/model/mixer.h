...
class Mix2Linear : public Mix2
{
  ...
  protected:
    inline std::int32_t idiv_signed32(std::int32_t val,std::int32_t s){return val<0?-(((-val)+(1<<(s-1)))>>s):(val+(1<<(s-1)))>>s;};
    inline void upd_w(std::int32_t d,std::int32_t rate) {std::int32_t wd=idiv_signed32(rate*d,PBITS);w=std::clamp(w+wd,0,std::int32_t(WSCALE));};
};
...
class NMixLogistic
{
  ...
  protected:
    inline std::int32_t idiv_signed32(std::int32_t val,std::int32_t s){return val<0?-(((-val)+(1<<(s-1)))>>s):(val+(1<<(s-1)))>>s;};
    inline std::int32_t idiv_signed64(std::int64_t val,std::int64_t s){return val<0?-(((-val)+(1<<(s-1)))>>s):(val+(1<<(s-1)))>>s;};
    inline void upd_w(std::int32_t i,std::int32_t wd){w[i]=std::clamp(w[i]+wd,-WRANGE,WRANGE-1);}
};
