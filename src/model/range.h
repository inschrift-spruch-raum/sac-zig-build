...
public:
  explicit RangeCoderBase(BufIO& buf, std::int32_t dec = 0):
    buf(buf),
    decode(dec) {};
...
inline std::uint32_t SCALE_RANGE(std::uint32_t& range, std::uint32_t& p1) {
  return (
    (static_cast<std::uint64_t>(range) * ((PSCALE - p1) << (32 - PBITS))) >> 32
  );
}

inline void RANGE_ENC_NORMALIZE(
  BufIO& buf, std::uint32_t& low, std::uint32_t& range, std::uint32_t TOP,
  std::uint32_t BOT
) {
  while(
    (low ^ (low + range)) < TOP
    || (range < BOT && (((range = -static_cast<std::int32_t>(low) & (BOT - 1)), 1) != 0))
  ) {
    buf.PutByte(low >> 24);
    range <<= 8;
    low <<= 8;
...
inline void RANGE_DEC_NORMALIZE(
  BufIO& buf, std::uint32_t& low, std::uint32_t& range, std::uint32_t& code,
  std::uint32_t TOP, std::uint32_t BOT
) {
  while(
    (low ^ (low + range)) < TOP
    || (range < BOT && (((range = -static_cast<std::int32_t>(low) & (BOT - 1)), 1) != 0))
...
public:
  ...
  std::function<void(std::uint32_t, std::int32_t)> encode_p1 =
    [this](std::uint32_t p1, std::int32_t bit) {
      EncodeBitOne(p1, bit);
      return;
    }; // stupid C++
  std::function<std::int32_t(std::uint32_t)> decode_p1 =
    [this](std::uint32_t p1) { return DecodeBitOne(p1); };
